...
def main():
    ...
    app = Publication_SDC(root,  c, ax,fig, cbar, cax)
...
class Publication_canvas_thickness(Frame):
 ...
 #set colorbar num
    def set_colorbar_ticknum(self, ticknum, decimalnum):
        cb, ele, canvas = self.cbar, 'mA', self.canvas
        c = self.c
        if decimalnum != 0:
            # cb.set_ticklabels()
...
class Publication_para_thickness(Frame):
    def __init__(self, master, canvas):
        ...
        self.figTitle_cb = ttk.Combobox(f_figTitle, values = ['mA', 'value range','mA + range', 'mA + high value', 'without title'], width = 20)
        ...
        self.colorbarTitle_cb = ttk.Combobox(f_colorbar_title, values = ['mA', 'value range','mA + range', 'without title'], width = 16)
    ...
    def get_colorbar_title(self, decimal):
        ...
        if s == 'mA':
            colorbarTitle = ['mA' for i in eles]
        elif s == 'value range':
