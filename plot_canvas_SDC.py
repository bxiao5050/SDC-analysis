...
from vertical_drag_1_SDC import RangeDrag_SDC
from xrangeDrag_SDC import XRangeDrag_SDC
from curve_overview_SDC import Curve_overview_SDC
class Plot_canvas_SDC(Frame):
    ...
    def on_dragrange(self):
        if self.rangedrag is None:
            self.rangedrag = RangeDrag_SDC(self, ax = self.ax)
            self.b_xrange.config(state = 'normal')
        self.canvas.draw()

    def on_x_dragrange(self):
        if self.x_rangeDrag is None:
            self.x_rangeDrag = XRangeDrag_SDC(self, ax = self.ax)
        self.canvas.draw()
    ...
    def set_xrange_project(self, x_range):
        if x_range is None:
            return
        if self.x_rangeDrag is None:
            self.x_rangeDrag = XRangeDrag_SDC(self, ax = self.ax, color = 'yellow', startPos_left = x_range[0],startPos_right = x_range[1])
        self.canvas.draw()

    #for porject import
    def on_dragrange_project(self, vline_value=None):
        if self.rangedrag is None:
            self.rangedrag = RangeDrag_SDC(self, ax = self.ax, color = 'green', vline_value=vline_value)
        self.canvas.draw()
